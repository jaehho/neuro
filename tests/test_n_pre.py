...
class TestParamsBroadcasting:
    ...
    def test_length_mismatch_errors(self) -> None:
        with pytest.raises(ValueError, match="r_pre has length 3"):
...
class TestSimulateSmoke:
    ...
    def test_simulate_runs(self, n_pre: int, tmp_path: Path) -> None:
        ...
        for key in series_keys(n_pre):
            assert key in df.columns, f"Missing series column: {key}"
        spike_types = set(pl.read_parquet(run.spikes)["spike_type"].unique().to_list())
        assert "post" in spike_types or run.spikes_written == 0

...
\begin{enumerate} % TODO: Add the relevant equations next to each variable for clarity
    \item \textbf{Membrane Potential ($V_i(t)$):} The fast microscopic integration of synaptic currents, evolving on the timescale $\tau_m$.
    \item \textbf{Presynaptic Trace ($x_j(t)$):} A fast, low-pass filtered record of presynaptic spiking, tracking the LTP window ($\tau_+$).
    \item \textbf{Postsynaptic Trace ($y_i(t)$):} A fast, low-pass filtered record of postsynaptic spiking, tracking the LTD window ($\tau_-$).
    \item \textbf{Eligibility Trace ($E_{ij}(t)$):} A slow mesoscopic buffer bridging the STDP coincidence with delayed reward, decaying over $\tau_e$.
    \item \textbf{Baseline Reward ($\bar{R}(t)$):} A slow macroscopic expectation of network performance, evolving over $\tau_{\bar{R}}$.
    \item \textbf{Synaptic Weight ($w_{ij}(t)$):} The slow macroscopic efficacy of the connection, driven by the product of the eligibility trace and the reward prediction error.
\end{enumerate}
